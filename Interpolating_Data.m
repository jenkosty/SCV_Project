...
%     interp_argo_data(i) = struct('lat',argo_lats_lons(i,1),'lon',argo_lats_lons(i,2),'month',argo_months(i),'year',argo_years(i), 'salt', interp_argo_salt(i,:), 'temp', interp_argo_temp(i,:));
% end
...
for i = 1:length(SO_argo)
    if isempty(SO_argo(i).salt) == 0
        argo_data_w_den(u) = struct('lat', SO_argo(i).lat, 'lon', SO_argo(i).lon,'den',gsw_rho(SO_argo(i).salt,SO_argo(i).temp,zeros(length(SO_argo(i)),1)) - 1000, 'salt', SO_argo(i).salt, 'temp', SO_argo(i).temp, 'month', SO_argo(i).time(2), 'year', SO_argo(i).time(1));
...
for i = 1:length(argo_data_w_den)
    ...
    if length(b) > 2
        ds_interp_argo_data(u) = struct('lat', argo_data_w_den(i).lat, 'lon', argo_data_w_den(i).lon,'salt',interp1(argo_data_w_den(i).den(b), argo_data_w_den(i).salt(b),density_grid),'temp', interp1(argo_data_w_den(i).den(b), argo_data_w_den(i).temp(b),density_grid), 'month', argo_data_w_den(i).month, 'year', argo_data_w_den(i).year);
        u = u + 1;
    end
end